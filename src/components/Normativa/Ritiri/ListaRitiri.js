...
import {getRitiroSuccess, searchRitiri} from "../../../appRedux/actions";
...
import SearchRitiri from "./SearchRitiri";

function ListaRitiri(props) {
  ...
  const listRitiri = useSelector(({ritiriStore}) => ritiriStore.listRitiri);
  ...
  useEffect(() => {
    if (location.state === undefined) {
      dispatch(searchRitiri(search));
      dispatch(showSpinner("Ricerca richiami in corso"));
    }
  }, []);

  useEffect(() => {
    if (listRitiri !== undefined) {
      setLista(listRitiri);
      dispatch(hideSpinner("Ricerca richiami completata"));
      if (listRitiri.length == 0) {
