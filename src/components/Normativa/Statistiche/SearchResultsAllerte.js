...
function  SearchResults () {
  ...
  const searchFilters = useSelector(({statisticheAllerteStore}) => statisticheAllerteStore.searchFilters);

  useEffect(() => {
    if (datiStatistiche !== null && datiStatistiche !== undefined) {
      dispatch(hideSpinner("Generazione statistica allerte completata"))
      setIsShown({pie:true});
  ...
  useEffect(() => {
    //console.log("---- SEARCH RESULTS - search filters updated")
  ...
  const onClick = (record,singleStat) => {
    ...
    dispatch(statisticheAllerteDettaglio(detailSearchFilters))


    //let dettaglioStatAllerta = localStorage.getItem(hash) 
    //if ( dettaglioStatAllerta === null) {
      //dispatch(statisticheAllerteDettaglioSuccess())
    //} else {
      //dispatch(statisticheAllerteDettaglioSuccess(JSON.parse(dettaglioStatAllerta)))
    //  dispatch(statisticheAllerteDettaglioSuccess())
    //}
    //alert()
  }
