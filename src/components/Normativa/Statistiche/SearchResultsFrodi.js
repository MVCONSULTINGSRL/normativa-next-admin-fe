...
import {
  statisticheFrodiDettaglio,
  hideSpinner
} from "../../../appRedux/actions";

function  SearchResultsFrodi () {
  ...
  const searchFilters = useSelector(({statisticheFrodiStore}) => statisticheFrodiStore.searchFilters);

  useEffect(() => {
    if (datiStatistiche !== null && datiStatistiche !== undefined) {
      setIsShown({pie:true});
      dispatch(hideSpinner("Generazione statistica frodi completata"))
  ...
  const onClick = (record,singleStat) => {
    ...
    dispatch(statisticheFrodiDettaglio(detailSearchFilters))
...
export default SearchResultsFrodi;